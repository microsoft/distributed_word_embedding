...
namespace multiverso
{
	...
	{
		void complex::Train(int argc, char *argv[])
		{
			...
			ParameterLoader *parameter_loader =
				new (std::nothrow)ParameterLoader(option_, WordEmbeddings[0]);
		...
		void complex::StartCollectWordcountThread(
		...
		void complex::CreateMultiversoParameterTable(
		...
		void complex::PrepareMultiversoParameterTables(
		...
		int64 complex::GetFileSize(const char *filename)
		{
#ifdef _MSC_VER
			struct _stat64 info;
		...
		void complex::RemoveDoneDataBlock(
		...
		void complex::TrainNeuralNetwork()
		{
			std::queue<DataBlock*>datablock_queue;
		...
		void complex::LoadData(DataBlock *data_block,
		...
		void complex::Run(int argc, char *argv[])
		{
		...
		bool complex::ReadWord(char *word, FILE *fin)
		{
			int idx = 0;
			char ch;
			while (!feof(fin))
		...
		//Read the vocabulary file; create the dictionary
	    //and huffman_encoder according opt
		int64 complex::LoadVocab(Option *opt,
